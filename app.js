...
async function load(){try{const response=await fetch("projects.json",{cache:"no-store"});if(!response.ok)throw new Error(`HTTP ${response.status}`);const catalog=await response.json();if(!Array.isArray(catalog))throw new Error("projects.json non contiene un elenco valido");allProjects=catalog.map(clean).filter(p=>p.published);renderAll()}catch(error){console.error(error);$("#projectGrid").innerHTML='<div class="empty catalog-error"><h3>Catalogo non disponibile</h3><p>Avvia il sito tramite un server locale per consentire la lettura di projects.json.</p></div>';$("#projectGrid").style.display="block"}}
function renderAll(){renderStats();renderFeatured();renderFilters();apply();setupContact()}
function renderStats(){const cats=new Set(allProjects.map(p=>p.category).filter(Boolean));const values={"#statProjects":allProjects.length,"#statCategories":cats.size,"#statResources":allProjects.reduce((n,p)=>n+resourceCount(p),0),"#statFeatured":allProjects.filter(p=>p.featured).length,"#catalogueProjectCount":allProjects.length};Object.entries(values).forEach(([sel,val])=>{const el=$(sel);if(el)el.textContent=val})}
function renderFeatured(){let list=allProjects.filter(p=>p.featured);if(!list.length)list=allProjects.slice(0,3);$("#featuredGrid").innerHTML=list.map(p=>`<a class="featured-card" href="${esc(safeHref(p.url))}">${iconHtml(p,"featured-icon")}<span class="featured-category">${esc(p.category)}</span><strong>${esc(p.title)}</strong><small>${esc(p.short_description)}</small><span class="featured-action">Apri progetto ↗</span></a>`).join("")}
...
function setupHomepage(){
  ...
  const toggle=$("#navToggle"),nav=$("#mainNav");if(toggle&&nav){toggle.addEventListener("click",()=>{const open=nav.classList.toggle("open");toggle.setAttribute("aria-expanded",String(open))});nav.addEventListener("click",e=>{if(e.target.closest("a")){nav.classList.remove("open");toggle.setAttribute("aria-expanded","false")}})}
  document.querySelectorAll("[data-skill-search]").forEach(link=>link.addEventListener("click",()=>{setTimeout(()=>{const input=$("#projectSearch");if(input){input.value=link.dataset.skillSearch||"";activeCategory="Tutti";renderFilters();apply()}},50)}));
